  ...
  static async loadHistory() {
    try {
      ...
      downloadHistory = downloadHistory.filter(
        (item) => item && typeof item === "object" && item.url && item.date
      );
    ...
    const doRequest = (currentUrl, redirectsLeft) => {
      const request = https.get(currentUrl, (res) => {
        ...
        if (res.statusCode !== 200) {
          reject(
            new Error(
              `HTTP ${res.statusCode}: Failed to download from ${currentUrl}`
            )
          );
          return;
        }
        ...
        res.on("data", (chunk) => {
          ...
          if (
            onProgress &&
            total &&
            (downloaded - lastReported > 100000 || downloaded === total)
          ) {
    ...
    try {
      if (!bin.isZip) {
        ...
        const tmpZip = path.join(
          os.tmpdir(),
          `mihari_${bin.name}_${Date.now()}.zip`
        );
      ...
      console.error(
        `[Dependencies] Failed to download ${bin.displayName}: ${error.message}`
      );
...
async function checkForUpdate(retry = 0) {
  ...
  try {
    ...
    console.error(
      `[AutoUpdate] Update check failed (attempt ${retry + 1}): ${
        error.message
      }`
    );
    ...
    if (retry < AUTO_UPDATE.MAX_RETRIES) {
      console.log(
        `[AutoUpdate] Retrying in ${AUTO_UPDATE.RETRY_DELAY / 1000} seconds...`
      );
      await new Promise((resolve) =>
        setTimeout(resolve, AUTO_UPDATE.RETRY_DELAY)
      );
  ...
  return new Promise((resolve, reject) => {
    function doRequest(currentUrl, redirectsLeft) {
      ...
      const request = https.get(currentUrl, (res) => {
        ...
        file.on("finish", () => {
          console.log(
            "[AutoUpdate] Installer download complete:",
            AUTO_UPDATE.INSTALLER_PATH
          );
      ...
      request.setTimeout(60000, () => {
        // 60 second timeout for installer download
        request.destroy();
...
async function autoUpdate(mainWindow, quitAfter = false) {
  try {
    ...
    const shouldUpdate =
      AUTO_UPDATE.FORCE || compareVersions(latestVersion, version) > 0;
    if (!shouldUpdate) {
    ...
    console.log(
      `[AutoUpdate] Update available: ${version} -> ${latestVersion}`
    );
...
function startUpdateChecks(mainWindow) {
  ...
  setTimeout(() => {
    autoUpdate(mainWindow).catch((error) => {
      console.error(
        "[AutoUpdate] Background update check failed:",
        error.message
      );
    });
  }, 10000);
  ...
  updateCheckInterval = setInterval(() => {
    autoUpdate(mainWindow).catch((error) => {
      console.error(
        "[AutoUpdate] Periodic update check failed:",
        error.message
      );
    });
...
ipcMain.handle("open-file-location", async (event, filePath) => {
  ...
  if (result.success) {
    console.log(
      "Successfully opened location:",
      result.actualPath || "directory"
    );
  } else {
